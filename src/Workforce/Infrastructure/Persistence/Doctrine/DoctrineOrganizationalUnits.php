...
final readonly class DoctrineOrganizationalUnits implements OrganizationalUnits
{
    ...
    public function discover(WorkplaceId $workplaceId, string $term, int $limit): array
    {
        ...
        $localRows = $this->connection->fetchAllAssociative("SELECT u.*, COUNT(m.id) FILTER (WHERE m.active = TRUE) AS use_count, (COUNT(m.id) FILTER (WHERE m.active = TRUE) > 0 OR COALESCE(BOOL_OR(d.featured), FALSE))::int AS featured FROM workforce_organizational_units u LEFT JOIN workforce_unit_definitions d ON d.normalized_name = u.normalized_name AND d.active = TRUE LEFT JOIN workforce_swap_pools p ON p.organizational_unit_id = u.id LEFT JOIN workforce_swap_pool_memberships m ON m.swap_pool_id = p.id WHERE u.workplace_id = :workplace AND u.status <> 'inactive' AND (:empty OR u.normalized_name LIKE :term OR translate(lower(u.aliases::text), 'áéíóúüñ', 'aeiouun') LIKE :term) GROUP BY u.id ORDER BY use_count DESC, u.name LIMIT :limit", $parameters, $types);
