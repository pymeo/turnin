...
final readonly class DoctrineWorkerAssignmentWriter implements WorkerAssignmentWriter
{
    ...
    public function replacePrimary(WorkerAssignment $assignment, array $accesses): void
    {
        ...
        $this->connection->executeStatement('INSERT INTO workforce_worker_assignments (id, worker_id, workplace_id, staff_category_id, specialty_id, organizational_unit_id, functional_area, employer_id, primary_assignment, active, created_at, updated_at) VALUES (:id, :worker, :workplace, :category, :specialty, :unit, :area, :employer, TRUE, TRUE, :created, :updated)', ['id' => $assignment->id(), 'worker' => $assignment->workerId(), 'workplace' => (string) $assignment->workplaceId(), 'category' => $assignment->staffCategoryId(), 'specialty' => $assignment->specialtyId(), 'unit' => $assignment->organizationalUnitId(), 'area' => $assignment->functionalArea(), 'employer' => $assignment->employerId(), 'created' => $assignment->createdAt(), 'updated' => $now], ['created' => 'datetime_immutable', 'updated' => 'datetime_immutable']);
